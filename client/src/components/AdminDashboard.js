...
export const AdminDashboard = ({ Activejobs, activejob:{actjobs}}) => {
    useEffect(() => {
        Activejobs();
        console.log({actjobs});
...
<Fragment>
      ...
      <Container style={{margin:"0px"}}>
        <br />
        <br />
        <h2 style={{ fontWeight:"bold",color: '#0199fe', paddingLeft: '20px' , textAlign:"center",marginBottom:"20px"}}> Admin DashBoard</h2>
        <div
          className='row p-1'
          style={{
        ...
        >
          {actjobs.map((item) => {
            return (
              <>
                <AdminDashboardCard item={item} key={item.company} />
                {/* {console.log(item)} */}
              </>
            );
          })}
...
export default connect(mapStateToProps, { Activejobs })(AdminDashboard);
